...
static HevTaskIOSocketProtect g_protect_callback;

EXPORT_SYMBOL void
hev_task_io_socket_set_protect_callback (HevTaskIOSocketProtect callback)
{
    g_protect_callback = callback;
}
...
hev_task_io_socket_socket (int domain, int type, int protocol)
{
    ...
    if (fd >= 0 && g_protect_callback != NULL && !g_protect_callback (fd)) {
        close (fd);
        return -3;
    }
